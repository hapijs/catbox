...
internals.Connection.prototype.start = function(callback) {
    ...
    this.client = new Memcache(this.settings.locations, this.settings.options);
    callback();
};
...
internals.Connection.prototype.validateSegmentName = function(name) {
    ...
    if (name.indexOf('\t') !== -1) {
        return new Error('Includes tab character');
    }

    if (name.indexOf('\n') !== -1) {
        return new Error('Includes newline character');
    }
