...
internals.Policy.prototype.get = function (key, callback) {     // key: string or { id: 'id' }
    ...
    if (!callback || typeof callback === 'object') {
        const options = callback || { full: false };

        promise = new Deferred();
        promise.promise = promise.promise
        .catch((rawResult) => rawResult)
        .then((rawResult) => {

            const result = {};
            ['err', 'value', 'cached', 'report'].forEach((propName, index) => {

                result[propName] = rawResult[index];
            });

            let finalPromise;
            if (options.full === false) {
                if (!result.err) {
                    finalPromise = Promise.resolve(result.value);
                }
                else if (!result.cached) {
                    finalPromise = Promise.reject(result.err);
                }
                else {
                    finalPromise = Promise.resolve(result.value);
                }
                ...
                if (!result.err) {
                    finalPromise = Promise.resolve(result);
                }
                else if (!result.cached) {
                    finalPromise = Promise.reject(result);
                }
                else {
                    finalPromise = Promise.resolve(result);
                }
        ...
        callback = promise.callback({ full: true });
    }
