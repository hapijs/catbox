...
internals.PendingResponse = function (id, onDidSend) {
...
internals.PendingResponse.prototype.join = function () {
...
//    console.trace('!!!', err, value, cached, report)
    if (err) {
        if (!cached) {
            this.reject(err);
        }
...
internals.Policy.prototype.get = async function (key) {     // key: string or { id: 'id' }
    ...
    let pending = this._pendings[id];
    if (pending) {
        return await pending.join();
    }

    pending = this._pendings[id] = new internals.PendingResponse(id, (count, report) => {
...
internals.Policy.prototype._callGenerateFunc = function (pending, key, cached, report) {
    ...
    this.rule.generateFunc.call(null, key, async (generateError, value, ttl) => {
        ...
        try {
            if ((generateError && this.rule.dropOnError) || ttl === 0) {                                    // null or undefined means use policy
                ...
                await this.set(pending.id, value, ttl);      // Lazy save (replaces stale cache copy with late-coming fresh copy)
            }
        }
        catch (err) {
            return finalize(err);
...
internals.Policy.prototype._get = function (id, callback) {
    ...
    this._cache.get({ segment: this._segment, id }).then((value) => {
...
internals.Policy.prototype.set = async function (key, value, ttl) {
    ...
    const id = (key && typeof key === 'object') ? key.id : key;
    try {
        return await this._cache.set({ segment: this._segment, id }, value, ttl);
    }
    catch (err) {
        ++this.stats.errors;
        throw err;
