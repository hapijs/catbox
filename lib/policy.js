...
internals.Policy.prototype._generate = function (pending, key, cached, report) {
    ...
    if (!this._pendingGenerateCall[pending.id]) {                // Check if a generate call is already in progress
        ...
        try {
            ...
            delete this._pendingGenerateCall[pending.id];
            return this._send(pending, err, null, null, report);
        }
    }
    else {
        this._pendingGenerateCall[pending.id] = pending;
    ...
    this.rule.generateFunc.call(null, key, async (generateError, value, ttl) => {
        ...
        const finalize = (err) => {
            ...
            if (cached &&
                ...
                return this._send(pending, error, cached.item, cached, report);
            }

            return this._send(pending, error, value, null, report);       // Ignored if stale value already returned
        };
