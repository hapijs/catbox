...
internals.Policy.prototype._validate = function (id, cached, generateFunc, report, callback) {
    ...
    if (cached &&
        ...
        if (cached.ttl > 0) {
            ...
            setTimeout(timerFunc, this.rule.staleTimeout);
        }
    }
    ...
    generateFunc.call(null, function (err, value, ttl) {
        ...
        if (wasCallbackCalled) {
            self._set(id, value, ttl);                            // Replace stale cache copy with late-coming fresh copy
            return;
        }
        ...
        self._set(id, value, ttl);                                // Save to cache (lazy) and continue
        return callback(null, value, null, report);
    });
};


internals.Policy.prototype._set = function(id, value, ttl) {
    ...
    this.set(id, value, ttl);                           // Lazy save
};
