...
exports = module.exports = internals.Policy = class {
    ...
    async _get(pending, key) {
        ...
        if (this._cache) {
            try {
                var cached = await this._cache.get({ segment: this.segment, id: key.id });
            }
            catch (err) {
                report.error = err;
                ++this.stats.errors;
    ...
    async set(key, value, ttl) {
        ...
        try {
            await this._cache.set({ segment: this.segment, id: internals.id(key) }, value, ttl || internals.Policy.ttl(this.rule));
        }
        catch (err) {
            ++this.stats.errors;
            throw err;
    ...
    async drop(key) {
        ...
        try {
            await this._cache.drop({ segment: this.segment, id: internals.id(key) });
            return;
        }
        catch (err) {
            ++this.stats.errors;
