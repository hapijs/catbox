...
internals.Policy.prototype.get = function (key, callback) {     // key: string or { id: 'id' }
    ...
    this._get(id, function (err, cached) {
        ...
        if (!self.rule.generateFunc ||
            (err && !self.rule.generateOnGetError)) {
...
internals.schema = Joi.object({
    ...
    generateOnGetError: Joi.boolean(),
    ...
    .with('generateOnGetError', 'generateFunc')
...
internals.Policy.compile = function (options, serverSide) {
    ...
    rule.generateOnGetError = options.generateOnGetError !== undefined ? options.generateOnGetError : true;         // Defaults to true
