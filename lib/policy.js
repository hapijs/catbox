...
internals.Policy.prototype._generate = function (id, key, cached, report, callback) {
    ...
    try {
        this._getGenerateFunc(id).call(null, key, function (err, value, ttl) {
...
internals.Policy.prototype._getId = function (key) {
    return (key && typeof key === 'object') ? key.id : key;
};


internals.Policy.prototype._getGenerateFunc = function (key) {
    var id = this._getId(key);
    return this._generateFuncs[id] || this.rule.generateFunc;
};
...
internals.Policy.prototype.getOrGenerate = function (id, generateFunc, callback) {       // For backwards compatibility
    ...
    this._generateFuncs[this._getId(id)] = function (id, next) {
        self._generateFuncs[id] = null;
